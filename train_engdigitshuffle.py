...
# Filter labels 0-9 and combine both datasets
...
train_indices_orig = filter_labels_0_9(x_train_full, y_train_full)
test_indices_orig = filter_labels_0_9(x_test_full, y_test_full)

# Combine all valid samples
all_indices = []
all_labels = []
all_sources = []  # Track which file each sample comes from

for idx in train_indices_orig:
    all_indices.append(idx)
    all_labels.append(y_train_full[idx])
    all_sources.append('train')

for idx in test_indices_orig:
    all_indices.append(idx)
    all_labels.append(y_test_full[idx])
...
all_sources = all_sources[shuffled_idx]

# Split 4:1 (80% train, 20% test)
split_point = int(0.8 * total_samples)
train_mask = np.arange(split_point)
test_mask = np.arange(split_point, total_samples)

# Separate data for new train and test sets
train_sources = all_sources[train_mask]
train_indices = all_indices[train_mask]

test_sources = all_sources[test_mask]
...
print(f"  - From original train file: {np.sum(train_sources == 'train')}")
print(f"  - From original test file: {np.sum(train_sources == 'test')}")
print(f"Test samples: {len(y_test)} ({len(y_test)/total_samples*100:.1f}%)")
print(f"  - From original train file: {np.sum(test_sources == 'train')}")
print(f"  - From original test file: {np.sum(test_sources == 'test')}")

def sparse_data_generator_from_hdf5_spikes(data_sources, data_indices, y_labels, 
                                            batch_size, nb_steps, nb_units, max_time, shuffle=True):
    """ This generator takes a spike dataset and generates spiking network input as sparse tensors. 
    
    Args:
        data_sources: List of source identifiers ('train' or 'test') for each sample
        data_indices: List of indices in the original files
        y_labels: The labels (already filtered and combined)
    """
    
    labels_ = np.array(y_labels, dtype=int)
    ...
    while counter<number_of_batches:
        ...
        for bc, idx in enumerate(batch_index):
            # Determine which file this sample comes from
            if data_sources[idx] == 'train':
                original_idx = data_indices[idx]
                firing_times = x_train_full['times']
                units_fired = x_train_full['units']
            else:
                original_idx = data_indices[idx]
                firing_times = x_test_full['times']
                units_fired = x_test_full['units']
...
def compute_classification_accuracy(data_sources, data_indices, y_labels):
    """ Computes classification accuracy on supplied data in batches. """
    accs = []
    for x_local, y_local in sparse_data_generator_from_hdf5_spikes(data_sources, data_indices, y_labels,
...
def train(train_sources, train_indices, y_train, test_sources, test_indices, y_test,
          lr=1e-3, nb_epochs=10, eval_interval=10):
    ...
    for e in range(nb_epochs):
        local_loss = []
        for x_local, y_local in sparse_data_generator_from_hdf5_spikes(train_sources, train_indices, y_train,
        ...
        if (e + 1) % eval_interval == 0 or e == nb_epochs - 1:
            train_acc = compute_classification_accuracy(train_sources, train_indices, y_train)
            test_acc = compute_classification_accuracy(test_sources, test_indices, y_test)
            train_acc_hist.append((e+1, train_acc))
            test_acc_hist.append((e+1, test_acc))
            
            print(f"Epoch {e+1}: loss={mean_loss:.5f}, train_acc={train_acc:.3f}, test_acc={test_acc:.3f}")
            ...
            if test_acc > best_test_acc:
                ...
                import time
                best_model_path = f'model/best_model_epoch{e+1}_testacc{test_acc:.3f}.pth'
                save_model(
                    best_model_path,
                    w1, w2, v1,
                    loss_hist=loss_hist,
...
loss_hist, train_acc_hist, test_acc_hist = train(
    train_sources, train_indices, y_train,
    test_sources, test_indices, y_test,
    lr=2e-4, 
    nb_epochs=nb_epochs,
    eval_interval=10
)
...
def get_mini_batch(data_sources, data_indices, y_labels, shuffle=False):
    for ret in sparse_data_generator_from_hdf5_spikes(data_sources, data_indices, y_labels,
                                                       batch_size, nb_steps, nb_inputs, max_time, shuffle=shuffle):
        return ret 

x_batch, y_batch = get_mini_batch(test_sources, test_indices, y_test)
...
plt.savefig('voltage_traces.png')
plt.close()
